    ...
    for (int ct0 = activation.size() - 3; ct0 > 0; ct0--)
        for (int ct = 0; ct < activation[ct0].size(); ct++)
            for (int ct1 = 0; ct1 < activation[ct0 - 1].size(); ct1++)
                error[ct0][ct1] += weight[ct0][ct][ct1] * actFD(zValue[ct0][ct]) * error[ct0][ct];


    for (int ct = 0; ct < activation.back().size(); ct++)
        for (int ct1 = 0; ct1 < activation[activation.size() - 2].size(); ct1++)
            weight.back()[ct][ct1] -= activation[activation.size() - 2][ct1] * sactFD(zValue.back()[ct]) * error.back()[ct] * LEARNING_RATE;

    for (int ct0 = activation.size() - 3; ct0 > 0; ct0--)
        for (int ct = 0; ct < activation[ct0 + 1].size(); ct++)
            for (int ct1 = 0; ct1 < activation[ct0].size(); ct1++)
                weight[ct0][ct][ct1] -= activation[ct0][ct1] * actFD(zValue[ct0][ct]) * error[ct0][ct] * LEARNING_RATE;

    for (int ct = 0; ct < weight[0].size(); ct++)
        for (int ct1 = 0; ct1 < weight[0][ct].size(); ct1++)
            weight[0][ct][ct1] -= activation[0][ct1] * actFD(zValue[1][ct]) * error[0][ct] * LEARNING_RATE;
...
/*
    ...
    for (int ct0 = activation.size() - 3; ct0 > 0; ct0--)
        for (int ct = 0; ct < activation[ct0].size(); ct++)
            for (int ct1 = 0; ct1 < activation[ct0 - 1].size(); ct1++)
                error[ct0][] += weight[][][] * actFD(zValue[][]) * error[][];
